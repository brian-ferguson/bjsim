...
class Visualizer:
    ...
    def plot_average_trajectory_with_extremes(self, avg_trajectory: List[float], 
                                            best_trajectory: List[float],
                                            worst_trajectory: List[float],
                                            hours_played: int, hourly_ev: float, 
                                            starting_bankroll: float) -> go.Figure:
        """
        Plot the average trajectory with best and worst case overlays.
        """
        hours = np.arange(0, len(avg_trajectory))
        expected_trajectory = starting_bankroll + (hours * hourly_ev)
        ...
        # Add worst case trajectory (light red, behind others)
        fig.add_trace(go.Scatter(
        ...
        # Add best case trajectory (light green, behind others)
        fig.add_trace(go.Scatter(
            x=hours[:len(best_trajectory)],
            y=best_trajectory,
            mode='lines',
            name='Best Case',
            line=dict(color='rgba(34, 197, 94, 0.6)', width=2),
            opacity=0.7
        ))
        
        # Add average actual trajectory (main line, on top)
        final_avg_profit = avg_trajectory[-1] - starting_bankroll
        expected_profit = expected_trajectory[-1] - starting_bankroll
        ...
        fig.update_layout(
            title=f'Performance Analysis: Average vs Best/Worst Cases',
            xaxis_title='Hours Played',
            yaxis_title='Bankroll ($)',
            hovermode='x unified',
            showlegend=True
